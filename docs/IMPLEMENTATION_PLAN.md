...
> **Current status:** Round 2 provides tested core primitives plus a durable SQLite application service and portable backup/restore, an interface prototype, and a packaged extension client. The acceptance checkpoint below is not met: desktop service integration, browser pairing, scheduled execution, full UI acceptance-flow E2E coverage, and signed cross-platform releases remain.
...
- SQLite will be authoritative once the host repository is implemented; JSON is reserved for typed structured columns. The proposed schema uses FTS5 and optional embedding blobs so packaging does not require a platform-native vector extension.
