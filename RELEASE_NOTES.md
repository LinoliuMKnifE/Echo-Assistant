...
- Native first-run detection that cannot be bypassed by inherited Luma WebView state, with secure OpenAI credential handoff to the live sidecar.

## Verification status

The repository's formatting, lint, typecheck, workspace build, 70 Vitest tests plus 2 Node package checks, 10 browser E2E tests, and 10 Rust tests pass. Local unsigned packages are diagnostic inputs, not public release artifacts; authoritative hashes are generated by each release workflow only after signature and packaged-runtime verification pass. Windows installer installation, AMO signing and live Firefox interoperability, macOS packaging, signing, and notarization remain release blockers.
